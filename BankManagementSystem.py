...
class Admin:
    ...
    def Show_Users(self):
        if len(self.Bank.Users) > 0:
            ...
            for user in self.Bank.Users:
                print(f"Name : {user.Name} , User ID: {user.Account_ID} , Email: {user.Email} , Address: {user.Address} , Account_Type: {user.Account_Type}")
            print()
        else:
    ...
    def Bankrupt(self, switch):
        if switch == 'YES':
            ...
            print("\n The Bank not crupted by the Admin\n")
...
Islami_Bank = Bank("Islami Bank", "Chittagong Bahaddarhat")
admin_Email = 'admin'
admin_password = "123"
admin = Admin(Islami_Bank)
register = RegistationAndLogin()

while True:
    print(f"\nWelcome to the {Islami_Bank.Name} \n")
    ...
    if option == "1":
            ...
            while True:
                ...
                if Option == "1":
                    ...
                    if Account_type == '1':
                        ...
                        break
                    register.Registation(Islami_Bank, newUser)
                elif Option == "2":
                    admin.Show_Users()
                elif Option == "3":
                    admin.Check_Bank_BL()
        ...
        if len(Islami_Bank.Users) > 0:
            print("For Login As User \n")
            UserName = input("Enter Your Email: ")
            Password = input("Enter Your Password: ")
            LogIn = register.Longin(Islami_Bank, UserName, Password)
            if LogIn:
                while True:
                    ...
                    if Option == "1":
                        ...
                        user.deposit_Money(Islami_Bank, amount)
                    elif Option == "3":
                        amount = int(input("\nHow Much Want to Withdraw: "))
                        user.withdraw_Money(Islami_Bank, amount)
                    elif Option == "4":
                        emailOfReciver = input("\nGive Email Of the Receiver: ")
                        amountSend = int(input("\nGive the amount :"))
                        user.transfer_money(
                            Islami_Bank, amountSend, emailOfReciver)
                    elif Option == "5":
                        user.show_Transaction_History()
                    elif Option == "6":
                        loanamount = int(input("\nGive the amount: "))
                        user.take_loan(Islami_Bank, loanamount)
                    elif Option == "7":
